...
const dui_usuario = document.querySelector('#dui_usuario');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    formulario.addEventListener('submit', function (e) {
        e.preventDefault();
        limpiarCampos();
        if (dui_usuario.value == '') {
            errorDui.textContent = 'EL DUUI ES REQUERIDO';
        } else if (nombres.value == '') {
            errorApellido.textContent = 'EL NOMBRE ES REQUERIDO';
...
function editarUsuario(idUsuario) {
    ...
    http.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            id.value = res.id;
            dui_usuario.value = res.dui_usuario;
